...
def SM3_birthday_attack(n, num_threads):
    cipher = set()

    def worker(thread_id):
        start_range = (thread_id * 2 ** 62) // num_threads
        end_range = ((thread_id + 1) * 2 ** 62) // num_threads

        while True:
            list_r = random.randint(start_range, end_range)
            ...
            j = text[:n]
            if j in cipher:
                print("线程 %d：攻击成功！碰撞值：%s" % (thread_id, j))
                break
            else:
                cipher.add(j)
...
start = time.time()
SM3_birthday_attack(8, 4)  # 使用4个线程进行攻击
end = time.time()
print("时间：%.3f秒" % (end - start))
